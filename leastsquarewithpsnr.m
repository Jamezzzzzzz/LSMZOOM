...
M = imread('original100x100.png.png'); % Replace with your filename
if size(M, 3) == 3
    M = rgb2gray(M);
end
M = double(M);

N = imread('100_100after (1).png'); % Replace with your filename
if size(N, 3) == 3
    N = rgb2gray(N);
end
N = double(N);
...
% 计算相对误差 η
[m, n] = size(M);
relative_error = norm(M- final_output) / norm(M);

% 显示结果
disp('平均相对误差 η：');
disp(relative_error);
disp(psnr(M/255,final_output/255));